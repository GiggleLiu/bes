...
class PercoTest(object):
    def __init__(self):
        ...
        mu=0.5
        self.model=QuantumHall(t=0.5,mu=mu,lamb=0.5)
    ...
    def test_symm(self):
        ...
        ion()
        cfunc=lambda k:Ck(*eigh(hgen.Hk(k)),T=0.05)
        kpath=path_k([kspace.G,kspace.special_points['X'][0],kspace.M[0],kspace.G],N=50)
        ck_red=perco.reduce_Ck(cfunc,kpath)
    ...
    def test_bandrecover(self):
        perco=self.get_perco(L=1,size=1,form='#')
        T=1.
        hgen=self.model.hgen
        kspace=perco.get_kspace()
        ion()
        cfunc=lambda k:Ck(*eigh(hgen.Hk(k)),T=T)
        kpath=path_k([kspace.G,kspace.special_points['X'][0],kspace.M[0],kspace.G],N=50)
        ck_red=perco.reduce_Ck(cfunc,kpath)
        hkmesh=Hmesh(C2H(ck_red))
        ekmesh=hkmesh.getemesh()
